# -*- coding: utf-8 -*-
"""
Created on Tue Apr  9 17:27:11 2019
...
def fit2(X_train,y_train,kernel_,c,M,S):
    ...
    while(True):        
        ...
        if (len(error) == 0):
            break
        elif (len(M)-len(sv))<=0 or len(error) <= len(M)-len(sv):
            print('GRR')
            break
        else:
            error=error[:(len(M)-len(sv))]        #print(error)
...
def getdata():
    ...
    best_c,bestkernel,bestWS=kfold_validate(X_train,y_train)
    accuracy=svm(X_train,y_train,X_test,y_test,best_c,bestkernel,bestWS)
    ...
    print('Best working set size is {}'.format(bestWS))
        ...
        for e in range(-4,-1,2):
            for WS in range(6,16,2):
                ...
                for train_index, test_index in skf.split(X, y):
                    ...
                    acc=svm(X_train,y_train,X_test,y_test,c,kernel,WS)
                    accuracy.append(acc)
