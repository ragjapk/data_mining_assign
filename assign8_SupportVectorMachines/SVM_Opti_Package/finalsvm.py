...
def plot_data_with_labels2(X_train,y_train,X_test,y_test,c,alphas,bias,kernel,sv):
    x_min=X_test[:, 0].min() - 1
    x_max =X_test[:, 0].max() + 1
    y_min = X_test[:, 1].min() - 1
    y_max = X_test[:, 1].max() + 1
...
def getdata():
    ...
    np.savetxt('f_data5.csv',sv,delimiter=',')
    #plot_data_with_labels2(X_train,y_train,X_test,y_test,best_c,alphas,bias,'L',sv)
